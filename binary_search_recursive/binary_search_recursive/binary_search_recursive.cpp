// binary_search_iterative.cpp : Defines the entry point for the console application.
...
int binarySearch(int *array, int target, int min, int max)
{
	...
	if (target < array[midpoint])
		binarySearch(array, target, min, midpoint);
	else if (target > array[midpoint])
		binarySearch(array, target, midpoint, max);
	else if (target == array[midpoint])
		return midpoint;
}
